...
下面是当前**已实现并可在真机使用**的能力（`devecocli check lint` 0 缺陷，纯逻辑均有单测）。

### 连接与配对

- 扫码或**粘贴 Base64URL 配对载荷**（`dsh-mobile-v1`，`hello.protocol = 3`），严格校验载荷版本/时效/地址
- 设备长期 token 存入 **Asset Store Kit**，可「快捷重新连接」而不必重新扫码
- 双通道（control + conversation）；`hello.capabilities` 能力门控，缺能力走 fallback 而非假设
- 请求超时 / 失败**只失效该请求**，不断开连接；传输层失败才重连
- 已保存多个端点并**手动切换**；可选「自动切换」按当前网络在局域网/公网之间择优
- 公网可走 Cloudflare 命名隧道（见下文「非局域网访问」）

### 工作区与会话

- 工作区切换、会话列表（含搜索）、新建会话、重命名、归档
- 会话按工作区归属归类，桌面端按路径创建的「未分组」会话也不会被藏掉
- **子会话（subagent session）不出现在会话列表**：宿主会把它当普通会话列出，客户端按
  `origin = subagent` / `parentSessionId` 过滤掉——它们不是用户打开的对话，宿主也不提供其历史
- Agent preset（含 PTC 模式）在会话头显示

### 对话

- 历史分页加载 + 实时事件流合并（按 `seq` 去重、按 generation/requestId 丢弃迟到响应）
- 发送消息、排队消息与队列管理（编辑 / 删除 / Steer）、停止生成
- Markdown 子集渲染、思考指示、对话 / 轨迹双视图、会话统计（token、上下文压力、缓存命中）
- Human-in-the-loop：审批卡片与问题卡片（含 plan 复核卡片）
- 图片发送与按需下载（客户端 JSON 帧预算 3.5 MB）
- `@` 文件引用补全（`file-list`）
- 任务列表（todos）与目标（goal）面板

### Plan 模式（依赖 dsh-plan-toggle 插件）

- **Plan 控件只在装了 `dsh-plan-toggle` 时才出现**；未装则不显示，也不退回官方 `/plan`
- 点击**当帧翻转**芯片（本地意图先于往返），待宿主投影确认后落到对应颜色；不再需要等一个
  公网往返（实测约 1.3s）才有反馈
- 切换未确认期间芯片**呼吸灯**（透明度往返 + 落色渐变）；局域网一个往返只有 30–50ms，
  因此设了 900ms **最小可见时长**，否则动效肉眼不可见
- 复核入口：`/plan-review` 流水线按钮 + 「复核中」进度 + 结果文本（含归档路径/发现数）

### 命令与设置

- `/` 命令与技能菜单（服务端驱动）：会话重命名/归档、导出、模型与推理强度、权限预设、目标、
  反馈、技能加载（如 `/eval`）等
- 模型 / 推理强度 / 权限预设选择器；权限变更后自动重取
- 主题（跟随系统/深色/浅色）、composer 快捷键开关、「@ 文件」与斜杠菜单显隐

### 网络健壮性（已修的典型问题）

- 会话相关的**错误帧不带 `sessionId`** 时按 requestType 认领在飞请求：宿主的错误约 30ms 就能
  显示，而不是挂到 30s 假超时
- 断线/网络切换自动重连；迟到响应按序号丢弃，不覆盖新状态
...
MVP 已在模拟器（HarmonyOS 6.1.1 / API 24）与真机（HUAWEI Mate 70 Pro+）端到端跑通：配对 → 双通道连接 →
工作区/会话列表 → 历史 + 实时对话 → 发送消息 → 新建会话 → 凭据持久化与自动重连。

在此之上本轮补齐（详见「功能特性」）：Plan 模式控件与复核入口、Plan 芯片点按即翻转 + 呼吸灯、
多端点保存/切换与公网隧道、子会话过滤、错误帧快速认领。

- 阶段 0 协议实测：`docs/spikes/2026-09-10-stage0-protocol-findings.md`
- 单元测试：`entry/src/test/`（纯逻辑：解码、严格 Base64URL、请求分道/超时/去重、历史合并、
  对话投影、分页、统计口径、Plan 投影与意图、会话过滤等）
